...
@objc extension ProcessInfo {
    static func shortNameForMinorVersion(_ minorVersion: Int) -> String? {
        if (minorVersion == 13) {
            return "High Sierra"
        }
        if (minorVersion == 14) {
    ...
    static var versionShortString: String {
        ...
        return "\(osVersion.majorVersion).\(osVersion.minorVersion)\(patchString)"
    }
    
    static var versionShortName: String {
        let osVersion = ProcessInfo.processInfo.operatingSystemVersion
        if let shortName = ProcessInfo.shortNameForMinorVersion(osVersion.minorVersion) {
            return shortName
        }
        return ProcessInfo.versionShortString
    }

    static var versionName: String {
        return "macOS \(ProcessInfo.versionShortName)"
    }
}
