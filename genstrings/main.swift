...
class GenStrings {
    ...
    func perform(path: String? = nil, outputPath: String? = nil, referencePath: String? = nil) {
        ...
        var newProcessedStrings = String()
        for string in sortedStrings {
            if let existingTranslation = stringsDict[string], existingTranslation != string {
                processedStrings.append("\"\(string)\" = \"\(existingTranslation)\"; \n")
                continue
            }
            if var referenceTranslation = referenceDict[string], referenceTranslation != string {
                referenceTranslation = GBig.traditionalize(referenceTranslation)
                processedStrings.append("\"\(string)\" = \"\(referenceTranslation)\"; \n")
                continue
            }

            newProcessedStrings.append("\"\(string)\" = \"\(string)\"; \n")
        }
        processedStrings = processedStrings + newProcessedStrings
        if let out = outputPath {
            do {
                try processedStrings.write(toFile: out, atomically: true, encoding: .utf8)
            } catch {
                print(error)
            }
        } else {
...
if CommandLine.arguments.count > 2 {
    ...
    var referenceTranslation: String? = nil
    if CommandLine.arguments.count > 3 {
        referenceTranslation = CommandLine.arguments[3]
    }
    genStrings.perform(path: path, outputPath: output, referencePath: referenceTranslation)
} else {
    genStrings.perform()
}
